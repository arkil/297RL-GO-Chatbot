...
if __name__ == "__main__":
    ...
    CONSTANTS_FILE_PATH = 'constants_sarsa.json'
...
def run_round(state, warmup=False):
    ...
    agent_action_index, agent_action = sarsa_agent.get_action(state, use_rule=warmup)
    ...
    sarsa_agent.add_experience(state, agent_action_index, reward, next_state, done)
...
def train_run():
    ...
    """
    writer = SummaryWriter(comment="DQN-learning")
    ...
    while episode < NUM_EP_TRAIN:
        ...
        if episode % TRAIN_FREQ == 0:
            ...
            print('Episode: {} period_reward_total: {} Avg Reward: {}' .format(episode, period_reward_total, avg_reward))
            # Flush
            if success_rate >= success_rate_best and success_rate >= SUCCESS_RATE_THRESHOLD:
                sarsa_agent.empty_memory()
            # Update current best success rate
            if success_rate > success_rate_best:
                print('Episode: {} Avg Reward: {}' .format(episode, avg_reward))
                success_rate_best = success_rate
                sarsa_agent.save_weights()
            writer.add_scalar("deep_Sarsa", avg_reward, episode)
            period_success_total = 0
            period_reward_total = 0
            # Copy
            sarsa_agent.copy()
